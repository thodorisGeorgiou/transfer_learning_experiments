...
preTrainedDir = sys.argv[4]
preTrainSupervision = sys.argv[5]
# preTrainedDir = sys.argv[3]
# preTrainSupervision = sys.argv[4]

if modelType not in ["vc", "ds", "op", "cc"]:
	exit("Model type not supported")

train_dir = os.getcwd() + "/trainedFromCheckpointOnlyConvLayersTrainSetSize/"+str(numTrainExamples)+"_2/"+preTrainSupervision+"_flow/"+modelType+"/"+run
# train_dir = os.getcwd() + "/trainedFromCheckpointOnlyConvLayers/"+preTrainSupervision+"_flow/"+modelType+"/"+run
...
if __name__ == '__main__':
	...
	for gpu in range(numGpus):
		with tf.name_scope('tower_%d' % (gpu)) as scope:
			with tf.device('/gpu:%d' % gpu):
				...
				predictions = models.predictFlow(trainCode, batch_size//numGpus, log, useType="train", first=(gpu==0), addToCollection=False)
	...
	restorableVariables = tf.get_collection("restorableVariables")
	# restorableVariables = [v for v in tf.get_collection("restorableVariables") if ("fc3" not in v.name)]
	...
	restoreSaver = tf.train.Saver(toRestore)
